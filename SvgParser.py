...
class SvgParser():
    ...
    def createPtrList(self):
        ...
        for line in self.pathList:
            ...
            #self.log.log(5, "Path zVal: " + str(self.scene.items()[self.scene.items().index(ptr)].zValue()))
    ...
    def resize(self,drawRect=(150,150,150,150)):
        ...
        # path length somehow buggy. doesn't change by calling resize with different sizes
        # logging.info("Picture resized. Total path length: %.2f" % (self.pathLen))
        ...
        while ptr<len(ss):
            ...
            if ss[ptr].isalpha():
                ...
                if state=="h":
                    ...
                    ptr+=1
                    self.log.log(5, "unknow state "+  str(state))
        return
    ...
    def parseRect(self,node):
        ...
        self.log.log(5, ">> Rect " + str(w) + str(h) + str(x) + str(y))
        self.moveTo(x,y)
        self.lineTo(x+w,y)
        self.lineTo(x+w,y+h)
        self.lineTo(x,y+h)
    ...
    def parseLine(self,node):
        ...
        self.log.log(5, ">> Line"+str(x1) + str(y1) + str(x2) + str(y2))
    ...
    def parsePolygon(self,node):
        ...
        for p in points:
            if len(p)==0: continue
            xstr,ystr = p.split(',')
            self.log.log(5, ">>\t"+str(xstr) + str(ystr))
    ...
    def parseTextNode(self,n):
        ...
        if "transform" in attrs:
            self.log.log(5, "trans"+str(attrs))
            tf = [1,0,0,1,0,0]
    ...
    def parseTransform(self,node):
        ...
        if tfAttr != "":
            #print "trans",tfAttr
            trans = tfAttr.split()
            #self.log.critical("trans: " + str(trans))
            tf=[1,0,0,1,0,0]
            for t in trans:
                if "scale" in t:
                    p0 = tfAttr.find('(', tfAttr.find(t))
                    p1 = tfAttr.find(')', tfAttr.find(t))
                    tmp = list(map(float,tfAttr[p0+1:p1].replace(" ", "").split(",")))
                    #self.log.critical("tmp: " + str(tmp))
                    if (len(tmp) >= 2):
                        tf[0] = tmp[0]
                        tf[3] = tmp[1]
                    elif (len(tmp) >= 1):
                        tf[0] = tmp[0]
                        tf[3] = tmp[0]
                        #self.log.critical("Elif-branch, tmp.len=%d, tmp.val=%d", len(tmp), tmp[0])
                    else:
                        tf[0] = 1
                        tf[3] = 1
                        #self.log.critical("Else-branch")
                elif "translate" in t:
                    p0 = tfAttr.find('(', tfAttr.find(t))
                    p1 = tfAttr.find(')', tfAttr.find(t))
                    tmp = list(map(float,tfAttr[p0+1:p1].replace(" ", "").split(",")))
    ...
    def parseNode(self,node,deep):
        self.log.log(5, " "*deep + ">>" +str(node.nodeName) + str(node.nodeType))
        tf = None
        if node.nodeType==1:
        ...
        if node.nodeName=="path":
            ...
            self.log.log(5, " "*deep + "unknow" +str(node.nodeName))
        return tf
    
    def parseChildNodes(self,node,deep=1):
        self.log.log(5, " "*deep + "parse->" + str(node.nodeName))
        # escape marker
